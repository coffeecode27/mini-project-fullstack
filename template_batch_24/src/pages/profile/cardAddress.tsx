...
const CardAddress = ({dataProfile, dataUser }:any) => {
  const addAddressState = useSelector((state:any)=> state.addAddress.usersAddressData)
  ...
  return (
    ...
    <div className="mb-4 flex items-center justify-between">
      ...
      <Flowbite theme={{ theme: customTheme }}>
        ...
        <Modal show={props.openModal === 'form-elements'} size="3xl" popup onClose={() => props.setOpenModal(undefined)}>
          <Modal.Header />
          <Modal.Body>
            <CardAddAddress dataProfile={dataProfile}/>
          </Modal.Body>
        </Modal>
      </Flowbite>
    </span>
    ...
    <div className="flow-root">
      ...
      {dataAddress && dataAddress.map((item: any) => (
      <div key={item.userEntityId}>
      <li className="py-3 sm:py-4">
      <div className="flex items-center space-x-4">
        <div className="min-w-0 flex-1">
        {item.userAddress.map((address:any) => (
               <p key={address.addrId} className=" text-sm text-gray-900 break-words">
